...
Після своєї відповіді обов'язково(якщо ти дійсно написав відповідь, а не "На жаль, ця інформація відсутня в базі даних університету.") вкажи до трьох джерел з сторінками, які найбільш точно відповідають на питання користувача і які ти найбільше використав для надання відповіді.  Сторінки вказані лише перед кожним фрагментом. Якщо ти використав фрагменти у яких джерела однакові, а різні лише сторінки, то пиши один раз джерелоі потім перерахування сторінок через кому. Все що всередині наданих тобі фрагментів це пункти і інша нумерація, це НЕ СТОРІНКИ. Пиши джерела інформації у наступному форматі:
    Джерела інформації:
Посилання, ст. X-Х
...
def get_status(vectorstore: Chroma) -> str:
    ...
    return (
        ...
        f"Режим: {mode}\n"
        f"Модель: `{model}`\n"
        f"Контекстне вікно: {NUM_CTX} токенів\n"
        f"Фрагментів у базі даних: {doc_count}\n"
